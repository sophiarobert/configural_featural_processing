...
for thisBlock in blocks:
    ...
    trial_order = np.concatenate((shuffle([1, 2, 3, 4, 5, 6]),shuffle([1, 2, 3, 4, 5, 6]), shuffle([1, 2, 3, 4, 5, 6]),shuffle([1, 2, 3, 4, 5, 6])))
    trial_order = np.round(trial_order / 6 - 0.1)
    trialSame = np.concatenate((shuffle([0, 1, 2, 3]),shuffle([0, 1, 2, 3]),shuffle([0, 1, 2, 3])))
    ...
    for thisComponent in target_imgComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "target_img" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisComponent in probe_imgComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "probe_img" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
